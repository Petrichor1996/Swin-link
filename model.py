...
from datasets.seg_dataset import mydata

import visdom
from swin_retransformer import *
from torch.optim import lr_scheduler
from torch.utils.data import DataLoader
from metric.DICE import DiceLoss
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, num_classes=2,
        ...
        self.encon1=EncodeBLkshort(96,192)
        self.encon2 = EncodeBLkshort(192, 384)
        self.encon3 = EncodeBLkshort(384, 768)


        self.baseconv=Convblock(768,768)
    ...
    def forward(self, x):
        ...
        x1, H1, W1 = self.layer1(x, H, W)
        x2, H2, W2 = self.layer2(x1, H1, W1)
        x3, H3, W3 = self.layer3(x2, H2, W2)
        x4, H4, W4 = self.layer4(x3, H3, W3)
        B, L, C = x.shape
        e0 = x.reshape(B, H, W, C).permute(0, 3, 1, 2)

        B,L,C=x1.shape
        e1=x1.reshape(B,H1,W1,C).permute(0,3,1,2)

        B, L, C = x2.shape
        e2 = x2.reshape(B, H2, W2, C).permute(0, 3, 1, 2)

        B, L, C = x3.shape
        e3 = x3.reshape(B, H3, W3, C).permute(0, 3, 1, 2)

        B, L, C = x4.shape
        e4= x4.reshape(B, H4, W4, C).permute(0, 3, 1, 2)

        d4=self.baseconv(e4)
        d44=self.encon3(e2)
        d4 = self.Debl4(d4+d44)

        d33=self.encon2(e1)
        d3 = self.Debl3(d4 + d33)

        d22=self.encon1(e0)
        d2 = self.Debl2(d3 + d22)
        # d1=self.Debl1(d2+e1)

        x = self.decov1(d2)

        x = self.conv2(x)
        x = self.decov2(x, is_act=False)

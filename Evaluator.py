...
class Evaluator(object):
    ...
    def Mean_Intersection_over_Union(self):
        ...
        return MIoU
    def DSC(self):
        DSC = 2*np.diag(self.confusion_matrix) /  (np.sum(self.confusion_matrix, axis=1) + np.sum(self.confusion_matrix, axis=0))

        DSC = np.nanmean(DSC)  # 跳过0值求mean,shape:[21]
        return DSC
    def MPA(self):
        MPA = np.diag(self.confusion_matrix) / np.sum(self.confusion_matrix, axis=1)
        MPA = np.nanmean(MPA)  # 跳过0值求mean,shape:[21]
        return MPA
    ...
    def Class_IOU(self):
        ...
        return MIoU
    def Sen(self):
        sen=self.confusion_matrix[0][0] /(self.confusion_matrix[0][0] +self.confusion_matrix[0][1])
        return sen
    def PPV(self):
        ppv=self.confusion_matrix[0][0] /(self.confusion_matrix[0][0] +self.confusion_matrix[1][0])
        return ppv
    ...
    def _generate_matrix(self, gt_image, pre_image):
        gt_image=gt_image.cuda().data.cpu().numpy()
        pre_image=pre_image.cuda().data.cpu().numpy()
